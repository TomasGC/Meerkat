...
def test_security_scenario_via_explicit_security_cases():
    ...
    endpoint = Endpoint(
        path="/search",
        method=HTTPMethod.GET,
        params=[Parameter(name="q", param_type="query", data_type="string", required=True)],
        response_codes=[200, 400],
        file_path="handlers.go",
        line_number=1,
    )
    scenarios = generate_scenarios_for_endpoint(endpoint)
    sql_scenarios = [s for s in scenarios if "DROP" in str(s.input_combination.get("q", ""))]
    assert sql_scenarios, "SQL injection scenario must be generated"
